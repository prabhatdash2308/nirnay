...
export function DeleteBudgetDialog({ budget, baseCurrency, onConfirm, onCancel }: DeleteBudgetDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
      <div className="bg-card w-full max-w-sm rounded-2xl shadow-xl border border-border overflow-hidden p-6">
        <h3 className="text-lg font-medium tracking-tight mb-2">Delete Budget?</h3>
        <p className="text-sm text-muted-foreground mb-6">
          Are you sure you want to remove the limit for this category? Any transactions will remain untouched.
        </p>
        
        <div className="p-4 bg-muted/50 rounded-xl mb-6 flex justify-between items-center text-sm border border-border/50">
          <span className="font-medium mr-4">{budget.category}</span>
